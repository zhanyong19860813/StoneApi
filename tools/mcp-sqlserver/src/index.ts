...
async function getPool(): Promise<ConnectionPool> {
  ...
  if (!server || !database) {
    throw new Error(
      '请配置 SQLSERVER_CONNECTION_STRING，或同时设置 SQLSERVER_SERVER + SQLSERVER_DATABASE（可选 SQLSERVER_USER / SQLSERVER_PASSWORD）',
    );
  }
...
function assertSafeSelectInner(sqlText: string): string {
...
const mcpServer = new McpServer(
  ...
  {
    instructions:
      '连接 StoneApi 配套的 SQL Server 测试库。可列出表/视图、查看列信息、抽样行、列出存储过程及读取定义。自定义 SELECT 会外包 TOP 限制行数。仅用于只读分析。',
  },
);
...
mcpServer.registerTool(
  ...
  async ({ schemaName }) => {
    ...
    const req = p.request();
    if (schemaName?.trim()) {
      req.input('schema', sql.NVarChar(128), schemaName.trim());
...
mcpServer.registerTool(
  ...
  async ({ schemaName, tableName }) => {
    const p = await getPool();
    const req = p.request();
    req.input('s', sql.NVarChar(128), schemaName);
    req.input('t', sql.NVarChar(128), tableName);
    ...
    req2.input('s', sql.NVarChar(128), schemaName);
    req2.input('t', sql.NVarChar(128), tableName);
    const pk = await req2.query(
    ...
    return textResult({
      ...
      primaryKeyColumns: pk.recordset.map((r: { COLUMN_NAME: string }) => r.COLUMN_NAME),
    });
  },
);
...
mcpServer.registerTool(
  ...
  async ({ schemaName }) => {
    ...
    if (schemaName?.trim()) {
      ...
      req.input('schema', sql.NVarChar(128), schemaName.trim());
      const r = await req.query(
        `SELECT SCHEMA_NAME(o.schema_id) AS procedure_schema, o.name AS procedure_name, o.modify_date
...
mcpServer.registerTool(
  'sqlserver_get_procedure_definition',
  {
  ...
  async ({ schemaName, procedureName }) => {
    ...
    const req = p.request();
    req.input('s', sql.NVarChar(128), schemaName);
    req.input('n', sql.NVarChar(128), procedureName);
    const r = await req.query(
      `SELECT m.definition
    ...
    const row = r.recordset?.[0] as { definition?: string } | undefined;
    if (!row?.definition) {
...
mcpServer.registerTool(
  ...
  {
    description:
      '执行只读 SELECT：必须是单条 SELECT 语句（可含 JOIN/WHERE 等），禁止分号注释 exec 等；结果外包 TOP 限制行数。用于复杂抽样或对照过程逻辑。',
    inputSchema: {
